...
language.
A single mapping language can have multiple implementations for generating KGs. 
For example, the RDF Mapping Language (RML)~\cite{dimou_ldow_2014} has several
...
each offering distinct advantages for their own unique use cases 
(i.e. FlexRML~\cite{Freund2024FlexRML} for KG generation on IoT devices). 

%The RDF Mapping Language (RML)~\cite{dimou_ldow_2014} is one example of such a
...
when executing the mappings specified within an RML document,
...
makes it challenging to optimize the mappings described by the RML
document.
