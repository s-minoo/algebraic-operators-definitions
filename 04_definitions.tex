...
\todo{Min Oo, notice that I have changed this one as well. There are two languages now, one for selecting the context objects and one for selecting the values from the context objects. Additionally, the type-cast function is now part of the source type and there are two more sets of data objects to capture explicitly the types of objects in the results of the first and the second language, respectively. This changes still need to be reflected in the rest of the paper.}
	...
	\begin{itemize}
		...
		\item
		$\cbeval$ is a function $\cbeval : \symSetDataset \times \symSetContentA\! \times \symDataAcc'\! \rightarrow \overline{\symSetContentB}$, and
...
\begin{example}
	...
	$\symSetDatasetX{\mathsf{json}}$ is the infinite set of all possible JSON documents, both $\symSetContentAX{\mathsf{jp}}$ and $\symSetContentBX{\mathsf{jp}}$ are the infinite set of all possible elements that may occur within JSON documents (i.e., $\symSetContentAX{\mathsf{jp}} = \symSetContentBX{\mathsf{jp}}$),
...
\begin{example}
	...
	To define $\sourceTypeX{\mathsf{csv}} = \sourceTypeTupleX{\mathsf{csv}}$ as a source type for CSV-based data sources, we let $\symSetDatasetX{\mathsf{csv}}$ be the infinite set of all possible CSV files,
	$\symSetContentAX{\mathsf{jp}}$ is the infinite set of all possible rows of CSV files, and
	$\symSetContentBX{\mathsf{jp}} \subset \symAllStrings$ is the infinite set of strings that may be values within CSV files.
	...
	every CSV document~$\symDataset \in \symSetDatasetX{\mathsf{csv}}$, $\evalX{\mathsf{csv}}(\symDataset,\varepsilon)$ returns all rows of $\symDataset$%
		%, in the order in which they appear within $\symDataset$%
	, and for every string $c$ that is a column name in $\symDataset$, and every row $\dataObject \in \symDataset$, $\cbevalX{\mathsf{csv}}(\symDataset,\dataObject,c)$ is the value that
		the row~%
	$\dataObject$ has in column~$c$.
	$\typeCastX{\mathsf{csv}}$ maps all string in $\symSetContentBX{\mathsf{jp}}$ to \ttl{xsd:string} literals.
\end{example}

%The question of how data sources of any such type are actually accessed in practice is irrelevant for our formalization.

Now we are ready to define the source operator. While they are nullary operators, they are specified using three parameters: the data source ...
...
\begin{definition}
	...
	The \textbf{$(\queryExpr,\symAttrQueryMap)$-specific mapping relation obtained from $\dataSource$}, denoted by $\sourceOpDflt$, is the mapping relation $\mappingRel$ such that $\symAttrSubSet = \fctDom{\symAttrQueryMap}$ and
	\begin{align*}
