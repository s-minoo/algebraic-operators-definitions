...
% 	$\symSetDatasetX{\mathsf{json}}$ is
	...
	That is, for a JSON document $\symDataset \in \symSetDatasetX{\mathsf{json}}$ and a query $\queryExpr \in \symDataAccX{\mathsf{jp}}$, $\evalX{\mathsf{jp}}(\symDataset,\queryExpr)$ returns the elements of $\symDataset$ selected by $\queryExpr$. If $\queryExpr'\! \in \symDataAccX{\mathsf{jp}}'$, and assume $\dataObject \in \symSetContentAX{\mathsf{jp}}$ is a JSON object within $\symDataset$, then $\cbevalX{\mathsf{jp}}(\symDataset,\dataObject,\queryExpr')$ returns the elements of $\symDataset$ that can be reached from $\dataObject$ as per the traversal specified by $\queryExpr'\!$.
...
\begin{example}
	...
	every CSV file~$\symDataset \in \symSetDatasetX{\mathsf{csv}}$, $\evalX{\mathsf{csv}}(\symDataset,\varepsilon)$ returns all rows of~$\symDataset$%
		%, in the order in which they appear within $\symDataset$%
	, and for every such row
		$\dataObject \in \evalX{\mathsf{csv}}(\symDataset,\varepsilon)$
		%$\dataObject$
	and every string $c$ that is a column name in~$\symDataset$, $\cbevalX{\mathsf{csv}}(\symDataset,\dataObject,c)$ is the value that
		the row~%
...
\todo{Min Oo, notice that I decided to simply use the symbol $\queryExpr$ here, instead of $\queryExpr_{root}$. This change needs to be reflected in the rest of the paper.}
	...
	\begin{align*}
		...
		,
	\end{align*}
	where $\symDataSeq = \fctEval{\symDataset}{\queryExpr_{root}}$ and
	\begin{equation*}
		X_d = \bigtimes_{\attr \in \fctDom{\symAttrQueryMap}}
		\big\{ (\attr, \var) \ \big| \
		\var \in \fctCbeval{\symDataset}{\dataObject}{\queryExpr'} \text{ with }  \queryExpr'\! = \symAttrQueryMap(\attr) \big\}
		.
	\end{equation*}
\end{definition}
