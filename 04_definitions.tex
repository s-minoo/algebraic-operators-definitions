...
\begin{example}
	...
	$\sourceType$ is the source type from Example~\ref{ex:running_ex_source_type},
	$\queryExpr = \error$ (as shown in Example~\ref{ex:source_type_csv}), and
	$\symAttrQueryMap = \{\attr_{\textsf{id}}\rightarrow id,  \attr_{\textsf{name}} \rightarrow firstname, \attr_{\textsf{age}} \rightarrow age\}$
	generates a mapping relation $r$ such that it has a schema  where all the
	generated mapping tuples have the attributes $\symAttrSubSet = \{
		\attr_{\textsf{id}}, \attr_{\textsf{name}}, \attr_{\textsf{age}}\}$,
	and $r$ contains two mapping tuples $t_1, t_2 \in r $ as:
	\begin{align*}
