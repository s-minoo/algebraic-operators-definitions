...
\begin{example}
    ...
    $\symDataAcc = \{ \error \}$, 
    $\symDataAcc' = \{ \textsf{id},  \textsf{firstname},\textsf{lastname},\textsf{age}\}$, 
    ...
    $\typeCast$ can transform the CSV values in $\symSetContentB$ to RDF literals (e.g. 
    $\fctTypeCast{\ttl{Alice}}$ creates RDF literal $(\ttl{"Alice"}, \ttl{xsd:string})$).  
\end{example}
}
...
\todo[inline]{Min Oo, please modify the following example to adapt it to the terminology and formalism that have used above. Perhaps it is an idea to split the example into two: The first one is only about the running example and the source type for CSV files, including the use of its two languages, and can be moved directly after Example~\ref{ex:source_type_csv}. The second one, then, is only about the source operator. Of course, the first one should already use the root query and one of the context-object queries that are also used by the example source operator.}

\begin{example}
	\label{ex:source_operator}
	We provide a running example used throughout the rest of the paper starting
	with a CSV file with the header,
	\todo{Since the column names are not variables / symbols in the formalism, I would not use italic font for them; yet, they should be highlighted in some way. I propose to use 'textsf' with fontsize 'small'; e.g., \textsf{\small id}.}
	\todo{What kind of a notation is "[( ... )]"? I would simply write: "... with a header introducing columns named \textsf{\small id}, ..."}
	$H= [(id, firstname, lastname, age)]$,
	containing the following two
	\todo{Writing "2" here is not correct English. A number that is meant as a quantifier (i.e., not as a numeric value) should be written as a word, not using digits, unless the number is greater than 12. So, here you should write "two rows"!}
	rows of CSV data records on people as:
	\begin{equation*}
		\bigDataObject = [(1, \text{Alice}, \text{Lee}, 23), (2, \text{Bob}, \text{Malice}, 25)]
	\end{equation*}
	...
	which, when evaluated as $\eval(\queryExpr_{root})$, will return the 2 CSV records in $\bigDataObject$
	as data objects, and
	\todo{I wouldn't use question marks for attribute names.}
	$\symAttrQueryMap = \{?id \rightarrow id,  ?name \rightarrow firstname, ?age \rightarrow age\}$
	generates a mapping relation $r$  with two mapping tuples
	\todo{Also mention what the attributes of the relation are (i.e., the schema).}
	$t_1, t_2 \in r$ as:
	\todo{The values in these tuples must be \ttl{xsd:string} literals.}
	\begin{align*}
