...
\begin{definition}
	...
	The \textbf{$(\queryExpr_{root},\symAttrQueryMap)$-specific mapping relation obtained from $\dataSource$}, denoted by $\sourceOpDflt$, is the mapping relation $\mappingRel$ such that
	\begin{align*}
...
containing the following 2 rows of CSV data records as: 
\begin{equation*}
...
\begin{example}
    ...
    Let $r = \mappingRel$ be the mapping relation generated in Example~\ref{ex:source_operator},
    and $\symProjSet = \{?id, ?name\}$. 
    The mapping relation $r' = (\symAttrSubSet', \symMappingInst')$ generated after
    applying $\fctProjectOpDflt{r}$ has $\symAttrSubSet' = \{ ?id, ?name \}$,
    with 
    \begin{align*}
        \symMappingInst' &= \{ t'_1, t'_2\} \text{ such that } t'_1 = \{ ?id \rightarrow 1, ?name \rightarrow \text{Alice}\}, \text{ and } \\
...
The mapping relation generated in Example~\ref{ex:projection_operator} still 
lacks the attributes $\attr_s, \attr_p, \attr_o$, and  $\attr_g$ required 
for the generation of RDF dataset.
\end{example} 
...
Let $r = \mappingRel$ be a mapping relation with $\subjAttr, \predAttr, \objAttr, \graphAttr \in \symAttrSubSet$, and let
\begin{align*}
	...
	.
\end{align*}
The \textbf{RDF dataset generated from $r$} is the RDF dataset
\begin{equation*}
	D =
	\{ G_\mathrm{dflt} \}
	\cup
