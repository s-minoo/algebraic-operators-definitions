...
\vspace{-2em}
	\begin{minipage}[t]{0.43\textwidth}
		\centering
		\begin{lstlisting}[columns=fullflexible, language=sparql, label={lst:sparql_class_expand}, caption={Normalization step~\ref{item:NormalizationStep:ClassExpansion} (expand shortcuts for class IRIs%
...
\end{lstlisting}
    ...
    \begin{minipage}[t]{0.53\textwidth}
        \centering
        \begin{lstlisting}[columns=fullflexible, language=sparql, caption={Normalization step~\ref{item:NormalizationStep:POMDuplication} (duplicate multi-predicate-object maps
...
\vspace{-2mm}
\begin{lstlisting}[columns=fullflexible, language=sparql,  caption={Normalization step~\ref{item:NormalizationStep:ConstantsExpansion} (expand shortcuts
...
\begin{lstlisting}[columns=fullflexible, language=sparql, caption={Normalization step~\ref{item:NormalizationStep:JoinRemoval} (replace referencing object maps
...
\begin{lstlisting}[columns=fullflexible, language=sparql, caption={Normalization step~\ref{item:NormalizationStep:TMDuplication1} (duplicate triples maps that contain multiple 
...
\vspace{-2mm}
    \begin{minipage}[t]{0.5\textwidth}
\begin{lstlisting}[columns=fullflexible, language=sparql, caption={Normalization step~\ref{item:NormalizationStep:TMDuplication2}a (duplicate triples maps with subject maps in cases in which the associated pred\-i\-cate-ob\-ject maps may contain multiple graph~maps).},
...
\end{lstlisting}
    ...
    \hspace{2mm}
    \begin{minipage}[t]{0.5\textwidth}
\begin{lstlisting}[columns=fullflexible, language=sparql, caption={Normalization step~\ref{item:NormalizationStep:TMDuplication2}b (duplicate subject maps
   %that contain multiple graph~maps%
   %containing multiple graph~maps%
   to contain one graph map%
 ).},
label={lst:sparql_graphmap_norm}]
DELETE { ?sm rr:graphMap ?gm . }
INSERT { [] rml:logicalSource ?ls ;
            rr:subjectMap [
                 rr:reference ?ref ;
                 rr:template ?template ;
