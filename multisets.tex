...
We also define one more helper definitions on the restriction of the domain of
the mapping tuples to make the definitions of the algebraic mapping operators
more accessible to read.

\begin{definition}
	\label{def:tuple_restrict}
	Given a mapping tuple $\mappingTuple$ and a subset $A_{sub} \subseteq
		\mathrm{dom}(\mappingTuple)$ of attributes for which $\mappingTuple$ is
	defined, we write $\mappingTuple[A_{sub}]$ to denote the mapping tuple
	$\mappingTuple'$ that is the restriction of $\mappingTuple$ to $A_{sub}$;
	i.e., $\mathrm{dom}(\mappingTuple') = A_{sub}$ and $\mappingTuple'(a) =
